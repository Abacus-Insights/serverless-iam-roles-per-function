...
class ServerlessIamPerFunctionPlugin {
    ...
    generateInlinePolicy(functionObject, policyStatements) {
        //set log statements
        policyStatements[0] = {
            Effect: "Allow",
            Action: [
    ...
    generateManagedPolicies(functionObject, managedPolicies) {
        //set vpc if needed
        ...
        if ((functionObject.iamManagedPoliciesInherit || (this.defaultInherit && functionObject.iamManagedPoliciesInherit !== false))
            && !lodash_1.default.isEmpty(this.serverless.service.provider.iamManagedPolicies)) {
            for (const s of this.serverless.service.provider.iamManagedPolicies) {
                managedPolicies.push(s);
            }
        }
        //add iamRoleStatements
        if (lodash_1.default.isArray(functionObject.iamManagedPolicies)) {
